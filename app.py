...
dashboard_data = {
    ...
    "warning_msg": "💡 CLICK ANYWHERE ON THE MAP TO SET A CUSTOM STARTING POINT.",
    "detections": [],
...
def set_start_location():
    if dashboard_data['control_state'] != 'RUNNING':
        ...
        lng = float(data.get('lng', 78.65620))

        global active_waypoints
        active_waypoints = [(lat, lng)]
        ...
        dashboard_data['current_lat'] = lat
        dashboard_data['current_lng'] = lng
        dashboard_data['step_idx'] = 0
        dashboard_data['warning_msg'] = f"📍 NEW START POINT SET AT ({lat:.4f}, {lng:.4f})"
...
def handle_control():
    ...
    if cmd == 'START':
        ...
        if dashboard_data['step_idx'] > 0:
            ...
            dashboard_data['battery_pct'] = 100.0
            dashboard_data['material_level_pct'] = 100.0
            dashboard_data['warning_msg'] = "💡 CLICK ANYWHERE ON THE MAP TO SET A CUSTOM STARTING POINT."
        ...
        if not dashboard_data['returning']:
            ...
            if dashboard_data['distance_traveled_km'] <= 0.0 or dashboard_data['step_idx'] <= 0:
                ...
                history_entry = {
                    ...
                    "start_place": f"({dashboard_data['start_lat']:.4f}, {dashboard_data['start_lng']:.4f})",
